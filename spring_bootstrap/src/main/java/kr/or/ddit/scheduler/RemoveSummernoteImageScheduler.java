...
public class RemoveSummernoteImageScheduler {
	...
	public void fileRemove() throws Exception {
		...
		if (files != null) {
			for (File file : files) {
				existFile = false;
				existFile = (noticeDAOBean.selectNoticeByImage(file.getName()) != null) || (boardDAOBean.selectBoardByImage(file.getName()) != null) || (pdsDAOBean.selectPdsByImage(file.getName()) != null);
				if(existFile) {
