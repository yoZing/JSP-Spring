...
window.onload = function() {
    getPage("<%=request.getContextPath()%>/reply/listP.do?pno=${pds.pno}&page=" + replyPage);
...
// 	alert("reply regist btn");
	...
	$.ajax({
		url : "<%=request.getContextPath()%>/reply/registP.do",
		type : "post",
		...
		success : function(data) {
			...
			getPage("<%=request.getContextPath()%>/reply/listP.do?pno=" + pno + "&page=" + data); // 리스트 출력
			$('#newReplyText').val("");
		},
		error : function() {
			alert('댓글 등록을 실패했습니다.');
...
function replyRemove_go() {
	...
	$.ajax({
		url : "<%=request.getContextPath()%>/reply/removeP.do?rno=" + rno + "&page=" + replyPage + "&pno=${pds.pno}",
		type : "get",
		success : function(page) {
			alert('삭제되었습니다.');
			getPage("<%=request.getContextPath()%>/reply/listP.do?pno=${pds.pno}&page=" + page);
			replyPage = page;
		},
		error : function() {
			alert('삭제 실패했습니다.');
